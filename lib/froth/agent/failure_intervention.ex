defmodule Froth.Agent.FailureIntervention do
  ...
  def maybe_intervene({:error, error_text}, execution)
      when is_binary(error_text) and is_map(execution) do
    if should_intervene?(execution) do
      with {:ok, context} <- build_context(execution, error_text),
  ...
  def maybe_intervene(result, _execution), do: result
  ...
  defp should_intervene?(execution) when is_map(execution) do
    enabled?() and
      supported_tool?(execution[:name]) and
      is_binary(execution[:cycle_id]) and
      is_binary(execution[:tool_use_id]) and
      is_binary(execution[:session_id]) and
      is_binary(execution[:bot_id]) and
      is_integer(execution[:chat_id])
  end
  ...
  defp build_context(execution, error_text) do
    with %Cycle{} = cycle <- Repo.get(Cycle, execution[:cycle_id]) do
      ...
      {:ok,
       %{
         ...
         initial_intention: initial_intention(messages, execution),
         reply_to: reply_target(execution)
       }}
    else
  ...
  defp build_report(context) when is_map(context) do
    case llm_report(context) do
      {:ok, report} -> report
      _ -> fallback_report(context)
    end
  end

  defp llm_report(context) when is_map(context) do
    prompt = report_prompt(context)
    ...
    with {:ok, %{content: content}} <- response,
      ...
      {:ok, normalize_report(input, context)}
    end
  end

  defp post_report(context, report) when is_map(context) and is_map(report) do
    ...
    reply_markup = report_reply_markup(report, execution)
        ...
        with {:ok, message_id} <- sent_message_id(sent),
             ...
             {:ok, pending_ask} <-
               PendingAsks.create(%{
                 ...
                 config: report_config(context, report)
               }) do
          pending_ask =
  ...
  defp report_config(context, report) do
    execution = context.execution

    %{}
    |> Map.put("kind", "failure_intervention")
    |> Map.put("require_reply", true)
    |> maybe_put("system", execution[:system_prompt])
    |> maybe_put("model", execution[:model])
    |> maybe_put("tools", execution[:tools] || [])
    |> maybe_put("thinking", stringify_map(execution[:thinking] || %{}))
    |> maybe_put("effort", execution[:effort])
    |> maybe_put("tool_timeout_ms", execution[:tool_timeout_ms])
    |> maybe_put("tool_name", execution[:name])
    |> maybe_put("provider", execution[:provider])
    |> maybe_put("original_error", context.error_text)
  ...
  defp report_prompt(context) when is_map(context) do
    execution = context.execution
    ...
    Intention guess:
    #{context.initial_intention}

    Failed tool:
    #{execution[:name]}

    Tool invocation:
    #{format_invocation(execution[:name], execution[:input])}
  ...
  defp normalize_report(input, context) when is_map(input) and is_map(context) do
    interventions =
      input
    ...
    %{
      "intention" => normalized_field(input["intention"], context.initial_intention),
      "situation" => normalized_field(input["situation"], fallback_situation(context)),
      "invocation" => normalized_field(input["invocation"], fallback_invocation(context)),
      "expectation" => normalized_field(input["expectation"], fallback_expectation(context)),
      "irritation" => normalized_field(input["irritation"], fallback_irritation(context)),
      "designation" => normalized_field(input["designation"], fallback_designation(context)),
      "intervention" =>
        if(interventions == [], do: fallback_interventions(context), else: interventions)
    }
  end

  defp fallback_report(context) when is_map(context) do
    %{
      "intention" => context.initial_intention,
      "situation" => fallback_situation(context),
      "invocation" => fallback_invocation(context),
      "expectation" => fallback_expectation(context),
      "irritation" => fallback_irritation(context),
      "designation" => fallback_designation(context),
      "intervention" => fallback_interventions(context)
    }
  end
  ...
  defp report_reply_markup(report, execution) when is_map(report) and is_map(execution) do
  ...
  defp maybe_open_button(%{bot_id: bot_id, bot_username: bot_username, cycle_id: cycle_id})
  ...
  defp maybe_open_button(_execution), do: nil
  ...
  defp initial_intention(messages, execution) when is_list(messages) and is_map(execution) do
    messages
    |> Enum.find_value(fn
  ...
  defp fallback_situation(context) do
    execution = context.execution
    tool_turns = count_tool_turns(context.messages)

    "The cycle reached a failed #{execution[:name]} call after #{length(context.messages)} messages and #{tool_turns} tool turns."
  end

  defp fallback_invocation(context) do
    execution = context.execution
    format_invocation(execution[:name], execution[:input])
  end

  defp fallback_expectation(context) do
    case context.execution[:name] do
  ...
  defp fallback_irritation(context) do
    "The tool failed with #{truncate(context.error_text, @max_field_chars)}"
  end

  defp fallback_designation(context) do
    if repeated_invocation?(context.messages, context.execution[:name]) do
      "stubborn retry"
    else
  ...
  defp fallback_interventions(context) do
    suggestions =
      case context.execution[:name] do
        "run_shell" ->
          [
  ...
  defp reply_target(execution) when is_map(execution) do
    chat_id = execution[:chat_id]
    bot_id = execution[:bot_id]

    candidate =
      execution[:last_agent_message_id] ||
        execution[:reply_to]

    if is_binary(bot_id) and is_integer(chat_id) and is_integer(candidate) do
