defmodule Froth.Tools.PendingAskSupport do
  ...
  def maybe_put_send_message_opt(keyword, key, value), do: Keyword.put(keyword, key, value)
  ...
  def message_id(_sent), do: {:error, "telegram send_message did not return a message id"}
  ...
  def create_pending_ask(attrs, bot_id, chat_id, original_message_id)
    ...
    with {:ok, pending_ask} <- PendingAsks.create(attrs) do
      {:ok,
       maybe_refresh_pending_ask_message_id(pending_ask, bot_id, chat_id, original_message_id)}
    end
  end

  def format_error(%Ecto.Changeset{} = changeset), do: format_changeset_errors(changeset)
