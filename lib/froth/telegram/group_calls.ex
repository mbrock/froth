defmodule Froth.Telegram.GroupCalls do
  ...
  def group_call_join_parameters(audio_source_id, payload, opts \\ [])
      when is_integer(audio_source_id) and is_binary(payload) and
             is_list(opts) do
    %{
  ...
  """
  def join_video_chat_request(
        group_call_id,
        audio_source_id,
        payload,
        opts \\ []
      )
      when is_integer(group_call_id) and is_integer(audio_source_id) and
    ...
    request = %{
      ...
      "join_parameters" =>
        group_call_join_parameters(audio_source_id, payload, opts),
  ...
  def join_video_chat(session_id, group_call_id, opts \\ [])
      when is_binary(session_id) and is_integer(group_call_id) and
             is_list(opts) do
    ...
    with true <- is_pid(pid) or {:error, :invalid_pid},
         ...
         {:ok, participant_id} <-
           resolve_join_participant_id(session_id, group_call_id, opts),
         {:ok, audio_source_id, join_payload} <-
           await_group_join_payload(group_call_id, timeout),
         request <-
         ...
         {:ok, join_response_payload} <-
           extract_join_response_payload(tdlib_result),
         :ok <-
           Cnode.set_tgcalls_group_join_response(
             group_call_id,
             join_response_payload
           ) do
      {:ok,
  ...
  defp extract_join_response_payload(%{"text" => payload})
       when is_binary(payload) do
    {:ok, payload}
  end
  ...
  defp extract_join_response_payload(%{"@type" => "error"} = error) do
    {:error,
     {:tdlib_error, Map.get(error, "code"), Map.get(error, "message")}}
  end
  ...
  defp resolve_default_participant_id(session_id, group_call_id, opts)
    ...
    with {:ok, chat} <- fetch_group_call_chat(session_id, group_call_id, opts),
         {:ok, participant_id} <-
           extract_default_participant_id(chat, group_call_id) do
      {:ok, participant_id}
    end
  end
    ...
    case Keyword.get(opts, :chat_id) do
      chat_id when is_integer(chat_id) ->
        with {:ok, chat} <-
             ...
             true <-
               group_call_chat?(chat, group_call_id) or
                 {:error, :group_call_chat_mismatch} do
          {:ok, chat}
        end
  ...
  defp find_group_call_chat(session_id, group_call_id)
    ...
    with {:ok, %{"chat_ids" => chat_ids}} <-
      ...
      Enum.reduce_while(
        chat_ids,
        {:error, :group_call_chat_not_found},
        fn chat_id, _acc ->
          case Telegram.call(session_id, %{
                 "@type" => "getChat",
                 "chat_id" => chat_id
               }) do
            {:ok, chat} ->
  ...
  defp group_call_chat?(
         %{"video_chat" => %{"group_call_id" => group_call_id}},
         group_call_id
       ),
       do: true
  ...
  defp extract_default_participant_id(_, _),
    do: {:error, :missing_default_participant_id}
