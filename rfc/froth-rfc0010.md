# FROTH-RFC-0010: OPENING HORMUZ — A Private Maritime Security Framework
...
threat model is well-understood. It does not work against state
military forces. Somali pirates with RPGs and skiffs are not the
IRGC Navy with C-802 anti-ship missiles.
...
5. DRONES AND LOITERING MUNITIONS — ISR and strike from land, sea,
   ...
   targeting layer over the entire strait.

6. GEOGRAPHY — The traffic separation scheme forces all shipping
   through a channel roughly 6 nautical miles wide: two-mile
   inbound lane, two-mile buffer, two-mile outbound lane. Iranian
   islands and coastline bracket the channel. Reaction time from
   shore to shipping lane is measured in minutes. The Musandam
   peninsula (Oman) provides the only non-Iranian coastline on the
   strait itself.

## The Plan

A private operation to open Hormuz would require five capabilities
...
a) ARMED ESCORT VESSELS — Commercial offshore patrol vessels
   ...
   A minimum of 4 escort vessels supports a daily convoy cycle.

b) CONVOY ROUTING — Managed transit windows. Two convoys per day
   (one inbound, one outbound), timed to maximize separation and
   minimize exposure. Route selection varies daily. Speed governed
...
a) MARITIME DOMAIN AWARENESS — Commercial satellite imagery
   (Planet, Maxar) for wide-area surveillance. AIS tracking
   (already commercial). Synthetic aperture radar satellites for
   all-weather vessel detection. Commercial providers can deliver
   near-real-time pictures of every vessel in the strait.

b) AERIAL ISR — Long-endurance commercial drones (Textron
   Aerosonde, Schiebel Camcopter) operating from Musandam or
   Omani islands. Persistent overhead coverage of the convoy
   route. Radar and EO/IR payloads for surface search and
   periscope detection.

c) UNDERSEA SURVEILLANCE — Deployable acoustic sensor arrays
   (commercial sonobuoy derivatives, seabed hydrophones) in the
   traffic separation scheme. Detects submarine and small craft
   movement. This technology exists commercially for offshore
   asset protection.
...
b) SEEK STATE UMBRELLA — The escort force operates under a
   ...
   state provides the air/missile defense umbrella. This is
   Operation Earnest Will with a public-private split.

c) POINT DEFENSE ONLY — Escort vessels carry short-range air
   defense (RAM, Phalanx/CIWS equivalents). This handles
   drones and possibly terminal-phase cruise missiles but
   cannot intercept salvos at range. A private entity can buy
   CIWS. A private entity cannot buy a naval integrated air
   defense system.

The honest answer: Phase 4 cannot be solved privately. A private
force can clear mines, escort convoys, jam drones, and deter fast
attack craft. It cannot defeat a state's cruise missile arsenal.
The plan requires a state partner willing to provide air defense,
or it requires a political arrangement where the cruise missile
threat is managed through diplomacy (Omani channel) rather than
kinetics.
...
a) LIABILITY — A private MCM vessel that misses a mine, followed
   by a tanker explosion, creates a liability chain that no
   insurer will underwrite and no corporate entity will accept.

b) ESCALATION — A private escort vessel that fires on an IRGC
   fast attack boat starts a war. Who authorized it? Under whose
   rules of engagement? The Flying Tigers worked because they were
   fighting Japan, not harassing it. The ambiguity of the Hormuz
   situation — neither peace nor declared war — makes private ROE
   impossible to define without state backing.

c) STATE MONOPOLY ON VIOLENCE — The international order assumes
   that only states secure waterways. UNCLOS assigns
   responsibility to littoral states. A private force, no matter
   how capable, operates in a legal void unless a state claims it.

d) IRAN'S RESPONSE — Iran would treat a private escort force as
   a proxy. The diplomatic fiction ("it's not a state navy, it's
   a commercial security service") would last until the first
   engagement. After that it is a belligerent.

## What Would Actually Work

The plan that has historical support and legal viability is a
hybrid: the Gulf of Aden model for shipboard security, the
Earnest Will model for state escort, and commercial autonomous
MCM to fill the mine clearance gap. Specifically:
...
2. Expanded PCASP on every vessel transiting the strait, funded
   ...
   deters fast boat harassment and drone attacks at the ship
   level.
...
4. A commercial maritime domain awareness service providing
   ...
   drone, and sensor fusion. This is a SaaS product, not a
   military operation.

The guerrilla insight is that the most militarily effective
component — mine clearance — is also the most commercially
viable and the least politically provocative. An Omani company
operating robots that find and destroy mines is not a mercenary
navy. It is a marine services contractor. The mines do not care
who finds them. The insurance markets do not care who clears
them. The legal framework for commercial mine survey already
...
1. Would Oman agree? Their balancing act between Iran and the
   West is the foundation. A private MCM service operating from
   Musandam would need to be framed as Omani national interest,
   not as a Western proxy.

2. Who funds the capital expenditure? Lloyd's, the P&I clubs,
   the major tanker operators (NITC excluded), sovereign wealth
   funds of transit-dependent states (Japan, South Korea, India)?

3. Can commercial AUVs actually find Iranian mines in operational
   conditions — current, silt, shallow water, acoustic clutter?
   The technology works in exercises. Exercises are not wars.

4. What happens when Iran sinks an unmanned survey vessel? Is
   that an act of war? Against whom? The robot has no flag state
   feelings.

5. The precedent problem: if a private entity secures Hormuz,
   who secures Malacca? Bab el-Mandeb? The Panama Canal? The
   model either scales or it doesn't. If it scales, the
   international maritime order changes. If it doesn't, Hormuz
   is a one-off that proves the rule.
