defmodule Froth.ObjectStoreTest do
  ...
  test "put_file/3 and fetch/2 round-trip a stored object", %{
    root_dir: root_dir
  } do
    ...
    assert {:ok, stored} =
             ObjectStore.put_file("video/test/source.txt", source)

    assert stored.url ==
             "http://example.test/froth/objects/video/test/source.txt"

    assert {:ok, path} = ObjectStore.local_path("video/test/source.txt")
    assert File.read!(path) == "hello object store"

    assert {:ok, ^destination} =
             ObjectStore.fetch("video/test/source.txt", destination)
  ...
  test "normalize_key/1 rejects traversal keys" do
    ...
    assert {:error, :invalid_key} =
             ObjectStore.normalize_key("video/../../oops")
  end
end
